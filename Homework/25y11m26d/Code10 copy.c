/**
 * @brief IPv4地址合法性验证 - 优化版
 * 
 * 问题描述：
 * 判断输入的字符串是否为合法的IPv4地址
 * 合法IPv4格式：A.B.C.D，其中A、B、C、D ∈ [0, 255]，且无前导零
 * 
 * 算法分析：
 * 1. 格式检查：必须恰好有3个点分隔成4段
 * 2. 字符检查：每段必须全为数字字符
 * 3. 空段检查：每段至少有一个数字 (digitc > 0)
 * 4. 前导零检查：多位数不能以0开头（0除外）
 * 5. 范围检查：每段数值必须在[0, 255]范围内
 * 
 * 与Code10.c的区别：
 * - 增加digitc计数器，更严格检查空段情况
 * - 更完善的错误处理逻辑
 * 
 * 时间复杂度：O(n) - n为字符串长度，最大30
 * 空间复杂度：O(1)
 * 
 * 优化点：
 * - 边解析边验证，发现错误立即返回
 * - 使用digitc提前检测空段错误
 */
/*在基于Internet的程序中，我们常常需要判断一个IP字符串的合法性。合法的IPv4地址是这样的形式：A.B.C.D
其中A、B、C、D均为位于[0, 255]中的整数。为了简单起见，我们规定这四个整数中不会有前导零存在，如001这种情况。现在，请你来完成这个判断程序吧。

输入
输入由多行组成，每行是一个字符串，输入由End of file结束。字符串长度最大为30，且不含空格和不可见字符。

输出
对于每一个输入，单独输出一行。如果该字符串是合法的IP，输出YES，否则，输出NO

样例
输入
202.115.32.24
a.b.c.d
End of file

输出
YES
NO*/
#include <stdio.h>
#include <string.h>
int main() {
    char ip[31];
    
    while (scanf("%s", ip) != EOF) {
        if (strcmp(ip, "End") == 0) {
            break;
        }
        
        int nums[4] = {0, 0, 0, 0};
        int numc = 0;
        int dotc = 0;
        int valid = 1;
        int digitc = 0;
        
        for (int i = 0; ip[i] != '\0'; i++) {
            if (ip[i] == '.') {
                if (digitc == 0) {
                    valid = 0;
                    break;
                }
                dotc++;
                numc++;
                digitc = 0;
                if (numc > 3) {
                    valid = 0;
                    break;
                }
            } else if (ip[i] >= '0' && ip[i] <= '9') {
                nums[numc] = nums[numc] * 10 + (ip[i] - '0');
                digitc++;
                if (nums[numc] > 255) {
                    valid = 0;
                    break;
                }
            } else {
                valid = 0;
                break;
            }
        }
        
        if (dotc != 3 || numc != 3 || digitc == 0) {
            valid = 0;
        }
        
        if (valid == 1) {
            int len = 0;
            int zeroc = 0;
            for (int i = 0; ip[i] != '\0'; i++) {
                if (ip[i] == '.') {
                    len = 0;
                } else {
                    if (len == 0 && ip[i] == '0' && ip[i+1] != '.' && ip[i+1] != '\0') {
                        zeroc = 1;
                        break;
                    }
                    len++;
                }
            }
            if (zeroc == 1) {
                valid = 0;
            }
        }
        
        if (valid == 1) {
            printf("YES\n");
        } else {
            printf("NO\n");
        }
    }
    
    return 0;
}